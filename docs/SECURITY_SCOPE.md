...
- Repository-scoped secret exposure indicators, when raw secret values are not repeated in reports or logs

## Out of scope

- External targets and public network scanning
- Private repos not named in the current approved task
- Raw secrets, tokens, keys, credentials, session material, payment data, account email, or live sensitive logs
